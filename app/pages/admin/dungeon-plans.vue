<template>
  <div class="space-y-6">
    ...
    >
      ...
      >
        <div>
          ...
          <div class="text-xs font-bold text-slate-500 mt-1">
            新建副本计划、编排队伍与队员（每队 5 人）
          </div>
        </div>
        <div class="flex items-center gap-2">
          <div
            class="flex items-center gap-1 p-1 rounded-xl bg-slate-50 border border-slate-100"
          >
            <button
      ...
      <div class="p-6">
        <div v-if="loading" class="py-12 text-center text-slate-400 font-bold">
          加载中...
        </div>
        <div
          v-else-if="plans.length === 0"
          class="py-12 text-center text-slate-400 font-bold"
        >
          暂无计划
        </div>
        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          >
            <div class="flex items-start justify-between gap-4">
              <div class="min-w-0">
                ...
                <div
                  ...
                  class="mt-2 text-[10px] font-black text-amber-700"
                >
                  已逾期：建议重新安排时间
                </div>
                <div
                  v-if="p.status === 'canceled'"
                  class="mt-2 text-[10px] font-black text-rose-700"
                >
                  已作废：可删除该编排
                </div>
              </div>
            ...
            <div
              class="mt-4 text-sm text-slate-600 font-bold whitespace-pre-wrap break-words"
            >
              {{ p.notes || "—" }}
            </div>
                  ...
                  >
                    ...
                    <div class="space-y-1.5">
                      <div
                        ...
                        :key="`${p.id}_${gIndex}_${tIndex}_slot_${slotIndex}`"
                      >
                        <div
                        ...
                        >
                          <div class="min-w-0">
                            ...
                            <div
                              class="text-[8px] font-black flex items-center gap-1.5 mt-0.5 text-[#45a6d5]"
                            >
                              <span
                                >战
                                {{
                            ...
                            <div
                              v-if="t.members[slotIndex - 1].remark"
                              class="mt-0.5 text-[8px] font-bold italic truncate max-w-[120px] text-slate-400"
                            >
                        ...
                        >
                          <div
                            class="font-black text-[10px] truncate italic"
                            :class="
                              t.slot_remarks?.[slotIndex - 1]
                                ? 'text-emerald-400'
                                : 'text-rose-400'
                          ...
                          <div
                            class="text-[8px] font-black shrink-0 uppercase tracking-tighter"
                            :class="
                              t.slot_remarks?.[slotIndex - 1]
                                ? 'text-emerald-300'
                                : 'text-rose-300'
...
const planCardClass = (p) => {
  if (p.status === "completed")
    return "border-emerald-200 bg-emerald-50/40 hover:shadow-md hover:border-emerald-300";
  if (p.status === "overdue")
    return "border-amber-200 bg-amber-50/40 hover:shadow-md hover:border-amber-300";
  if (p.status === "canceled")
    return "border-rose-200 bg-rose-50/30 opacity-80 hover:shadow-md hover:border-rose-300";
...
const fetchPlans = async () => {
  loading.value = true;
  try {
    const res = await $fetch("/api/dungeon-plans", {
      query: { limit: 50, view: viewMode.value },
    });
...
const refreshMemberInList = async (p, gIndex, tIndex, mIndex) => {
  ...
  try {
    ...
    const res = await $fetch(`/api/dungeon-plans/${p.id}`, {
      method: "PUT",
      body: { groups },
    });
...
const openCreate = () => {
  ...
  form.value = {
    title: "",
    start_at: formatLocalInput(roundUpMinutes(new Date(), 60)),
    notes: "",
    groups: [],
    is_public: false,
    tags: [],
  ...
  if (Array.isArray(p.groups)) {
    groups = p.groups.map((g, idx) => ({
      ...
      teams: Array.isArray(g.teams)
        ? g.teams.map((t, j) => {
            const members = [null, null, null, null, null];
            ...
            return {
              ...
              title: t.title || "",
              note: t.note || "",
              slot_remarks: Array.isArray(t.slot_remarks)
                ? t.slot_remarks
                : ["", "", "", "", ""],
              members,
            };
...
const savePlan = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      const res = await $fetch(`/api/dungeon-plans/${editingId.value}`, {
        method: "PUT",
        body: payload,
      });
...
const setStatus = async (status) => {
  ...
  try {
    ...
    const res = await $fetch(`/api/dungeon-plans/${statusTarget.value.id}`, {
      method: "PUT",
      body,
    });
    if (!res?.success) throw new Error(res?.error || "更新失败");
...
const handleSlotClick = (gIndex, tIndex, mIndex) => {
  if (
    selectedSwap.value ||
    slotMember(form.value.groups[gIndex]?.teams[tIndex], mIndex)
  ) {
...
const saveMyMember = async (payload) => {
  ...
  try {
    const res = await $fetch("/api/my-members", {
      method: "POST",
      body: payload,
    });
    if (!res?.success) throw new Error(res?.error || "保存失败");
...
const updateMyMember = async (m) => {
  ...
  try {
    const res = await $fetch(`/api/my-members/${m.id}`, {
      method: "PUT",
      body: { remark: m.remark || null },
    });
    if (!res?.success) throw new Error(res?.error || "保存失败");
  } catch (e) {
    $alert("保存失败", e?.message || String(e));
