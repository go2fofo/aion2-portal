  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <img
            src="/bbbswz.png"
            class="w-full h-full object-contain group-hover:scale-110 transition-transform rounded-full"
          />
        </div>
    ...
    <main class="flex-1 relative z-20 flex flex-col items-center pt-8 h-full">
      ...
      >
        <!-- 装饰元素 -->
        <div
          class="absolute -top-6 -right-4 md:-right-12 text-3xl md:text-5xl animate-bounce"
        >
          ☀️
        </div>
        <img
          src="/xiaoniao.png"
          class="absolute top-4 -left-2 md:top-10 md:-left-20 w-10 h-10 md:w-16 md:h-16 -rotate-12 object-contain drop-shadow-md animate-bounce"
          style="animation-duration: 3s"
        />

        <h2
          class="relative text-3xl sm:text-5xl md:text-7xl font-black tracking-widest drop-shadow-[0_2px_0_#fff] md:drop-shadow-[0_4px_0_#fff] flex flex-col md:block items-center gap-2"
        >
          <span
        ...
        <div class="md:hidden">
          ...
          >
            ...
            >
              ...
              <img
                src="/xiaoniao.png"
                class="w-10 h-10 object-contain animate-bounce-short z-10"
              />
              <!-- 菜单展开指示器 -->
              <div
            ...
            <!-- 呼吸光晕 -->
            <div
              class="absolute inset-0 rounded-full border-2 border-[#f9b11d] opacity-50 animate-ping pointer-events-none"
              v-if="!showMobileMenu && !isDragging"
            ></div>
          </div>
            ...
            <div :key="activeTab" class="h-full">
              ...
              >
                ...
                >
                  ...
                  <div
                    v-if="loadingPosts"
                    class="flex-1 flex items-center justify-center text-slate-400 font-bold"
                  >
                    加载中...
                  </div>
                  <div
              ...
              <div
                ...
                class="grid grid-cols-1 md:grid-cols-3 gap-4 h-full"
              >
                <div
                  v-if="loadingPosts"
                  class="col-span-full flex items-center justify-center text-slate-400 font-bold"
                >
                  加载中...
                </div>
                <template v-else>
                  <div
                    v-for="post in posts"
                    :key="post.id"
                    class="p-4 bg-[#F0F9FF] rounded-2xl border-2 border-white hover:-translate-y-1 transition-transform"
                  >
                    <div class="text-4xl mb-2">🐼</div>
                    <p class="text-sky-900 font-bold line-clamp-2">
                      {{ post.title }}
                    </p>
                  ...
                  <div
                    v-if="posts.length === 0"
                    class="col-span-full flex flex-col items-center justify-center text-slate-400 py-10"
                  >
              ...
              >
                ...
                >
                  ...
                  <div class="flex flex-col">
                    ...
                    <span
                      class="text-xs font-bold text-yellow-500 italic font-mono text-lg leading-none"
                      >999,821</span
                    >
                  </div>
                </div>
                  ...
                  <div class="flex-1 w-full overflow-x-auto custom-scroll pb-1 sm:pb-0">
                    <div class="flex gap-2">
                      ...
                      <div class="relative inline-block text-left mr-2">
                        ...
                        <div
                          class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-yellow-600"
                        >
                          <svg
                ...
                <div v-else class="flex-1 overflow-y-auto custom-scroll p-2 -mx-2">
                      ...
                      >
                        ...
                        <div class="flex-1 min-w-0">
                          ...
                          <div class="text-xs text-slate-400 mt-1 truncate">
                            {{ member.title_name || member.note || "暂无签名" }}
                          </div>
                        </div>
                      </NuxtLink>
                    </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <span
                        class="text-[10px] sm:text-xs text-slate-500 font-medium mb-1 truncate w-full text-center"
    ...
    >
      ...
      >
        <path
          d="M0,40 C200,80 400,0 600,40 C800,80 1000,20 1200,60 L1200,120 L0,120 Z"
          fill="currentColor"
        />
      </svg>
      ...
      >
        <path
          d="M0,60 C300,20 600,100 900,40 C1050,10 1200,80 1200,80 L1200,150 L0,150 Z"
          fill="currentColor"
        />
      </svg>
      ...
      >
        <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M12 21V11M12 11C14 11 16 9 16 7C16 5 14 3 12 3C10 3 8 5 8 7C8 9 10 11 12 11Z"
            fill="#D8F5A2"
            stroke="#B2E455"
            stroke-width="2"
...
const toggleMenu = () => {
  // 防止拖拽结束时误触发点击
  if (
    Math.abs(btnPos.x - initialBtnPos.x) < 5 &&
    Math.abs(btnPos.y - initialBtnPos.y) < 5
  ) {
...
const spawnCloud = () => {
  ...
  for (let tries = 0; tries < 10; tries++) {
    ...
    if (!clouds.value.some((c) => overlaps(candidate, c))) {
      ...
      setTimeout(() => {
        clouds.value = clouds.value.filter((x) => x.id !== candidate.id);
      }, life);
      break;
    }
  }
...
const startDrift = () => {
  ...
  const step = (now) => {
    ...
    clouds.value.forEach((c) => {
      c.ox = +(
        Math.sin(t * (c.speedX || 0.06) + (c.phaseX || 0)) * (c.ampX || 8)
      ).toFixed(2);
      c.oy = +(
        Math.cos(t * (c.speedY || 0.07) + (c.phaseY || 0)) * (c.ampY || 6)
      ).toFixed(2);
    });
...
.animate-grass-wave {
  animation: grass-wave 8s ease-in-out infinite alternate;
}
.animate-sway {
...
.animate-speech-bubble {
  animation: speech-bubble 5s infinite;
}
.cloud-item {
  --tx: 0px;
  --ty: 0px;
  ...
  transform: translate(
      calc(var(--tx) + var(--offset-x, 0px)),
      calc(var(--ty) + var(--offset-y, 0px))
    )
...
.cloud-item.cloud-fade-leave-active {
...
.hero-figure {
  ...
  background: radial-gradient(
    ...
    rgba(174, 226, 249, 0.25) 70%,
    transparent 100%
  );
  box-shadow: 0 12px 24px rgba(69, 166, 213, 0.16);
}
.hero-img {
  filter: drop-shadow(0 8px 0 rgba(249, 177, 29, 0.3))
    drop-shadow(0 16px 0 rgba(69, 166, 213, 0.2));
}
...
.panel-cartoon {
  ...
  border-radius: 2rem;
  box-shadow: 0 12px 20px rgba(69, 166, 213, 0.12);
}
.panel-cartoon::after {
  content: "";
  position: absolute;
...
@keyframes walk-across {
  ...
  90% {
    transform: translateX(90vw) rotate(-5deg);
  }
  100% {
