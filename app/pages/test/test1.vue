...
<template>
  <div
    class="min-h-screen w-full bg-slate-50 text-slate-800 dark:bg-slate-950 dark:text-slate-100 py-8 transition-colors duration-200"
  >
    <!-- 撑满 95% 宽度的响应式主体容器 -->
    <div class="w-[95%] max-w-[1800px] mx-auto space-y-6">
      <!-- 顶部 Header -->
      <header
        class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5"
      >
        <div>
      ...
      <section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <!-- 左侧：时间预设选择 (占1列) -->
        <div
          class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col justify-between"
        >
          <div>
            <h2
              class="text-base font-semibold text-slate-900 dark:text-slate-100 mb-4 flex items-center gap-2"
            >
              <span>⏱️</span>
            ...
            <div class="space-y-3">
              ...
              >
                ...
                <span
                  class="text-[11px] text-slate-400 dark:text-slate-500 mt-1 font-mono"
                >
        ...
        <div
          class="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col"
        >
          <div class="flex items-center justify-between mb-3">
            <h2
              class="text-base font-semibold text-slate-900 dark:text-slate-100 flex items-center gap-2"
            >
              <span>📝</span>
      ...
      <section v-if="testResult.after" class="space-y-6">
        <!-- 结果状态 Banner -->
        <div
          class="flex flex-wrap items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 px-6 shadow-sm"
        >
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <span class="text-xs text-slate-500 dark:text-slate-400 ml-2"
                >基准时间: {{ testResult.executedTime }}</span
              >
            </div>
          </div>
          ...
          <div class="flex items-center gap-2">
            ...
            >
              <span
                ...
                :class="testResult.hasChanges ? 'bg-emerald-500' : 'bg-slate-400'"
              ></span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <!-- 刷新前卡片 -->
          <div
            class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm flex flex-col"
          >
            <div
              class="bg-slate-100/70 dark:bg-slate-800/50 px-5 py-3 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between"
            >
              <span
                class="font-semibold text-sm text-slate-600 dark:text-slate-300 flex items-center gap-2"
              >
                <span>📌</span> 刷新前原始数据
              </span>
            ...
            <div class="p-4 flex-1 bg-slate-50/50 dark:bg-slate-950/50">
              <pre
                class="font-mono text-xs text-slate-700 dark:text-slate-300 leading-relaxed overflow-x-auto max-h-[500px] p-2 whitespace-pre-wrap break-all"
                >{{ JSON.stringify(testResult.before, null, 2) }}</pre
              >
            </div>
          </div>
          ...
          <!-- 刷新后卡片 (高亮显示) -->
          <div
            class="bg-white dark:bg-slate-900 border-2 border-emerald-500/80 dark:border-emerald-500/60 rounded-2xl overflow-hidden shadow-sm flex flex-col"
          >
            <div
              class="bg-emerald-50 dark:bg-emerald-950/30 px-5 py-3 border-b border-emerald-100 dark:border-emerald-900/40 flex items-center justify-between"
            >
              <span
                class="font-semibold text-sm text-emerald-700 dark:text-emerald-400 flex items-center gap-2"
              >
                <span>✨</span> 刷新后结果数据 (点击右上角可一键导出)
              </span>
              <span class="text-xs font-mono text-emerald-600/70 dark:text-emerald-400/70"
                >After</span
              >
            </div>
            <div class="p-4 flex-1 bg-slate-50/50 dark:bg-slate-950/50">
              <pre
                class="font-mono text-xs text-slate-800 dark:text-slate-200 leading-relaxed overflow-x-auto max-h-[500px] p-2 whitespace-pre-wrap break-all"
                >{{ JSON.stringify(testResult.after, null, 2) }}</pre
              >
            </div>
          </div>
        </div>
        ...
        <!-- 校验断言指南 -->
        <div
          class="bg-blue-50/60 dark:bg-blue-950/20 border border-blue-200/80 dark:border-blue-900/40 rounded-2xl p-5 text-blue-900 dark:text-blue-200"
        >
          ...
          <ul
            class="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs leading-relaxed text-blue-800 dark:text-blue-300/90"
          >
            <li class="flex items-start gap-2">
              <span class="text-blue-500">•</span>
              <span
                ><strong>奥德能量 (interval)</strong>：非会员/会员每满 3 小时分别累加
                10/15 点，不超过动态上限。</span
              >
            </li>
            <li class="flex items-start gap-2">
              <span class="text-blue-500">•</span>
              <span
                ><strong>古树庆典/次元袭击 (daily)</strong>：跨越每日 05:00
                恢复对应固定次数，上限内正确累加。</span
              >
            </li>
            <li class="flex items-start gap-2">
              <span class="text-blue-500">•</span>
              <span
                ><strong>噩梦副本 (daily)</strong>：跨越每日 05:00 恢复 2
                次，受最大值与补充上限约束。</span
              >
            </li>
            <li class="flex items-start gap-2">
              <span class="text-blue-500">•</span>
              <span
                ><strong>觉醒战 / 战场 (weekly)</strong>：跨越每周三 05:00 恢复 3
                次或重置状态。</span
              >
            </li>
            <li class="flex items-start gap-2">
              <span class="text-blue-500">•</span>
              <span
                ><strong>圣域副本全系列 (weekly)</strong>：跨越每周三 05:00 通过自定义
                action 完美清空动态字典。</span
              >
            </li>
            <li class="flex items-start gap-2">
              <span class="text-blue-500">•</span>
              <span
                ><strong>微风商店与指令书 (weekly)</strong>：每周三 05:00
                统一清空服务器与角色的限购次数及状态。</span
              >
            </li>
            <li class="flex items-start gap-2">
              <span class="text-blue-500">•</span>
              <span
                ><strong>神器回廊 (artifact-cloister)</strong>：跨越周三/周六 22:10
                节点被重置为未完成状态。</span
              >
            </li>
          </ul>
