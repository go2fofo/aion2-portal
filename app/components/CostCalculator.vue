<template>
  <div class="h-full flex flex-col gap-6">
    ...
    <div class="space-y-4 shrink-0">
      <div
        class="flex flex-col md:flex-row gap-4 bg-white/50 backdrop-blur-sm p-4 rounded-3xl border-2 border-white"
      >
    ...
    <div class="flex-1 min-h-0">
      <div
        class="bg-white/80 backdrop-blur-sm rounded-[2.5rem] border-4 border-white shadow-xl overflow-hidden h-full flex flex-col relative"
      >
        <!-- 面板头部与汇率 -->
        <div
          class="p-6 md:p-8 bg-gradient-to-br from-sky-50 to-white border-b-2 border-sky-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0"
        >
          ...
          <!-- 汇率设置区 (在头部并列显示) -->
          <div
            class="bg-white/80 backdrop-blur-sm px-5 py-3 rounded-2xl border-2 border-sky-100 flex items-center gap-4 z-10 shadow-sm"
          >
          ...
          <!-- 装饰图标 -->
          <div
            class="absolute -bottom-4 -right-4 text-9xl opacity-5 pointer-events-none font-black italic"
          >
            B.B.B.S
          </div>
        </div>
        ...
        <div v-if="selectedEq" class="flex-1 flex flex-col min-h-0">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              >
                ...
                <div class="flex justify-between items-start mb-4">
                  <div
                    class="font-black text-slate-800 text-lg leading-tight truncate pr-4 flex-1 flex items-center justify-center"
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <div class="space-y-1.5">
                    <div class="flex justify-between items-center ml-1">
                      ...
                      >
                      <span
                        v-if="recentPrices[req.materialId]"
                        class="text-[9px] font-bold text-slate-400"
                        >⏱️ 近期:
                        {{ recentPrices[req.materialId].price_w }}w</span
                      >
                    </div>
                    <div class="relative">
                      <input
          ...
          <!-- 成本汇总 (固定在底部，左右排列) -->
          <div
            class="p-6 md:p-8 bg-slate-900 rounded-t-[2.5rem] text-white shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.3)] relative overflow-hidden group shrink-0"
          >
            <div
            ...
            >
              ...
              <div class="shrink-0 w-full md:w-auto">
                ...
                >
                  <span
                    v-if="recording"
                    class="w-5 h-5 border-4 border-slate-900/30 border-t-slate-900 rounded-full animate-spin"
                  ></span>
                  <span>记录本次计算</span>
                </button>
              </div>
            ...
            <!-- 背景光晕 -->
            <div
              class="absolute -top-20 -right-20 w-64 h-64 bg-sky-500 rounded-full blur-[80px] opacity-10 group-hover:opacity-20 transition-opacity"
            ></div>
            <div
              class="absolute -bottom-20 -left-20 w-64 h-64 bg-[#f9b11d] rounded-full blur-[80px] opacity-5 group-hover:opacity-10 transition-opacity"
            ></div>
          </div>
        </div>
...
const recordCalculation = async () => {
  ...
  try {
    ...
    // 更新回数据库
    const { error: updateError } = await supabase.from("site_config").upsert(
      {
