  ...
  <Teleport to="body">
    ...
    <Transition name="modal-fade">
      <div v-if="dialog.visible" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
        <!-- 背景遮罩 -->
        <div class="absolute inset-0 bg-slate-900/40 dark:bg-slate-950/60 backdrop-blur-sm transition-opacity" @click="handleMaskClick"></div>
        
        <!-- 弹窗主体 -->
        <div class="relative bg-white dark:bg-slate-900 w-full max-w-[320px] rounded-3xl shadow-[0_20px_60px_-10px_rgba(0,0,0,0.2)] dark:shadow-[0_20px_60px_-10px_rgba(0,0,0,0.6)] overflow-hidden transform transition-all animate-pop-in border border-slate-100 dark:border-slate-800">
          <!-- 顶部装饰 -->
          ...
          <div class="p-6 pt-8 text-center">
            ...
            <div class="mb-5 relative h-16 w-16 mx-auto">
               <div class="absolute inset-0 bg-slate-50 dark:bg-slate-800 rounded-full animate-ping opacity-20" :class="dialog.type === 'confirm' ? 'bg-yellow-400' : 'bg-blue-400'"></div>
               <img src="/xiaoniao.png" class="relative w-full h-full object-contain animate-bounce-short z-10" />
            </div>

            <h3 class="font-black text-xl text-slate-800 dark:text-slate-100 mb-3 px-2">{{ dialog.title }}</h3>
    ...
    <Transition name="fade">
      <div v-if="loading.visible" class="fixed inset-0 z-[9999] bg-white/90 dark:bg-slate-950/90 backdrop-blur-md flex items-center justify-center transition-opacity">
        <div class="flex flex-col items-center gap-6 animate-float">
          <div class="relative w-24 h-24">
             ...
             <!-- 旋转圆环 -->
             <div class="absolute inset-0 border-[6px] border-slate-100 dark:border-slate-800 rounded-full"></div>
          ...
          <div class="flex flex-col items-center gap-1">
            <span class="font-black text-slate-800 dark:text-slate-100 text-lg tracking-wide">{{ loading.text || '加载中' }}</span>
            <span class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Loading...</span>
          </div>
        </div>
      </div>
    </Transition>
